...
func TestRunStackActionReturnsSkippedServices(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			results, err := h.runStackAction(t.Context(), h.dockerCli, "stack", testCase.action, testCase.service, 0, true, slog.Default())
...
func TestRunStackActionRejectsMissingService(t *testing.T) {
	...
	for _, action := range []string{"scale", "restart", "run"} {
		t.Run(action, func(t *testing.T) {
			h := newStackActionTestHandler(t, services)
			results, err := h.runStackAction(t.Context(), h.dockerCli, "stack", action, "missing", 1, true, slog.Default())
...
func TestRunStackActionReturnsPartialResultsOnHardError(t *testing.T) {
	...
	results, err := h.runStackAction(t.Context(), h.dockerCli, "stack", "restart", "", 0, true, slog.Default())
