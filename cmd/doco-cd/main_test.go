...
func TestHandleEvent(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expectedStatusCode:   http.StatusInternalServerError,
			expectedResponseBody: `{"error":"deployment failed","content":"failed to deploy stack %[3]s: failed to load compose config: failed to load compose project: open %[2]s/compose.yaml: no such file or directory","job_id":"%[1]s"}`,
			customTarget:         "",
			swarmMode:            false,
		},
		{
