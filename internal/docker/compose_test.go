...
func TestDeployCompose(t *testing.T) {
	...
	repo, err := git.CloneRepository(tmpDir, p.CloneURL, p.Ref, c.SkipTLSVerification, c.HttpProxy, c.SSHPrivateKey)
	if err != nil {
...
func TestHasChangedConfigs(t *testing.T) {
	...
	repo, err := git.CloneRepository(tmpDir, url, git.MainBranch, c.SkipTLSVerification, c.HttpProxy, c.SSHPrivateKey)
	if err != nil {
