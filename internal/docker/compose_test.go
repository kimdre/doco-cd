...
func TestHasChangedConfigs(t *testing.T) {
	...
	project, err := LoadCompose(t.Context(), tmpDir, projectName, []string{"docker-compose.yml"}, []string{""}, []string{}, map[string]string{})
	if err != nil {
...
`
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			project, err := LoadCompose(ctx, tmpDir, projectName, []string{filePath}, []string{""}, []string{}, resolvedSecrets)
			if err != nil {
				t.Fatal(err)
			}
