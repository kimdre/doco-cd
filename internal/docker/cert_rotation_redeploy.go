...
func RotateProjectCertificates(
	...
	addComposeServiceLabels(selectedProject, deployConfig, payload, strings.TrimSpace(labels[DocoCDLabels.Source.URL]), ref.WorkingDir, app.Version, timestamp, ComposeVersion, latestCommit, projectHash)
...
func rotateSwarmProjectCertificates(
	...
	addSwarmServiceLabels(cfg, project, deployConfig, payload, strings.TrimSpace(labels[DocoCDLabels.Source.URL]), ref.WorkingDir, app.Version, timestamp, latestCommit, projectHash)
	addSwarmVolumeLabels(cfg, deployConfig, payload, ref.WorkingDir)
	addSwarmConfigLabels(cfg, deployConfig, payload, strings.TrimSpace(labels[DocoCDLabels.Source.URL]), ref.WorkingDir, app.Version, timestamp, latestCommit)
	addSwarmSecretLabels(cfg, deployConfig, payload, strings.TrimSpace(labels[DocoCDLabels.Source.URL]), ref.WorkingDir, app.Version, timestamp, latestCommit)
