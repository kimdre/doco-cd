...
func DeploySwarmStack(ctx context.Context, dockerCli command.Cli, project *types.Project, deployConfig *config.DeployConfig,
	...
	// Get images used in the stack
	services, err := swarmInternal.GetStackServices(ctx, dockerCli.Client(), opts.Namespace)
	if err != nil {
		return fmt.Errorf("failed to get stack services: %w", err)
	}

	beforeImages := make(map[string]struct{})

	for _, service := range services {
		if service.PreviousSpec != nil {
			beforeImages[service.PreviousSpec.TaskTemplate.ContainerSpec.Image] = struct{}{}
		}
	}

	err = swarmInternal.RunDeploy(ctx, dockerCli, &opts, cfg)
	if err != nil {
		return err
	}

	if deployConfig.PruneImages {
		// Get images used in the stack after deployment
		services, err = swarmInternal.GetStackServices(ctx, dockerCli.Client(), opts.Namespace)
		if err != nil {
			return fmt.Errorf("failed to get stack services after deployment: %w", err)
		}

		afterImages := make(map[string]struct{})
		for _, service := range services {
			afterImages[service.Spec.TaskTemplate.ContainerSpec.Image] = struct{}{}

			if service.PreviousSpec != nil {
				afterImages[service.PreviousSpec.TaskTemplate.ContainerSpec.Image] = struct{}{}
			}
		}

		afterDigests := make(map[string]struct{})

		for image := range afterImages {
			digest := imageDigest(image)
			if digest != "" {
				afterDigests[digest] = struct{}{}
			}
		}

		// Remove images that were used before deployment but are no longer used
		removeImages := make([]string, 0, len(afterImages))

		for image := range beforeImages {
			digest := imageDigest(image)

			if digest == "" {
				continue // skip images without digest
			}

			if _, found := afterDigests[digest]; found {
				continue // keep image if digest is present
			}

			removeImages = append(removeImages, image)
		}

		fmt.Println("beforeImages:", beforeImages)
		fmt.Println("afterImages:", afterImages)
		fmt.Println("removeImages:", removeImages)

		if len(removeImages) > 0 {
			err = RunImageRemoveJob(ctx, dockerCli, removeImages)
			if err != nil {
				return fmt.Errorf("failed to remove old images: %w", err)
			}
