...
type Config struct {
	...
	RecoverOnStartup              bool                   `env:"RECOVER_ON_STARTUP,notEmpty" envDefault:"true"`                                                   // RecoverOnStartup rebuilds reconciliation state (jobs and event listeners) for already-deployed repositories on process start from their existing local checkout, then heals drift (restarts unhealthy containers, redeploys fully-missing stacks) the same way a normal deploy would
