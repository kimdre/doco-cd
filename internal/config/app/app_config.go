...
type Config struct {
	...
	GitHttpAuthUser               string                 `env:"GIT_HTTP_AUTH_USER,notEmpty" envDefault:"oauth2"`                        // GitHttpAuthUser is the HTTP Basic Auth username paired with GIT_ACCESS_TOKEN for HTTP(S) clone/fetch (e.g. a GitLab deploy token username). Defaults to oauth2.
