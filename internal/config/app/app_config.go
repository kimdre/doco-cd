...
type Config struct {
	...
	RecoverOnStartup              bool                   `env:"RECOVER_ON_STARTUP,notEmpty" envDefault:"true"`                                                   // RecoverOnStartup rebuilds reconciliation state (jobs, listeners, startup recovery checks) for already-deployed repositories on process start, using only their existing local checkout (no network access)
