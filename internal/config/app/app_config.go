...
type Config struct {
	...
	RecoverOnStartup              bool                   `env:"RECOVER_ON_STARTUP,notEmpty" envDefault:"true"`                                                   // RecoverOnStartup rebuilds reconciliation state (jobs and event listeners) for already-deployed repositories on process start, using only their existing local checkout (no network access and no deployments)
