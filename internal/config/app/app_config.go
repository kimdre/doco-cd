...
type Config struct {
	...
	DockerSwarmConfigRetention    int                    `env:"DOCKER_SWARM_CONFIG_RETENTION,notEmpty" envDefault:"0" validate:"min=0"` // DockerSwarmConfigRetention is the global default number of old Swarm config revisions to keep per resource (excluding the active one)
	DockerSwarmSecretRetention    int                    `env:"DOCKER_SWARM_SECRET_RETENTION,notEmpty" envDefault:"0" validate:"min=0"` // DockerSwarmSecretRetention is the global default number of old Swarm secret revisions to keep per resource (excluding the active one)
