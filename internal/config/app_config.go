...
type AppConfig struct {
	...
	SelfUpdateEnabled           bool                   `env:"SELF_UPDATE_ENABLED,notEmpty" envDefault:"false"`                     // SelfUpdateEnabled enables the self-update endpoint and background image checks for the doco-cd container itself
	SelfUpdateInterval          int                    `env:"SELF_UPDATE_INTERVAL,notEmpty" envDefault:"0" validate:"min=0"`       // SelfUpdateInterval is the interval in seconds between scheduled self-update checks; 0 disables the scheduler while keeping the API trigger available when enabled
	SelfUpdateWaitHealthy       bool                   `env:"SELF_UPDATE_WAIT_HEALTHY,notEmpty" envDefault:"true"`                 // SelfUpdateWaitHealthy waits for the replacement container health check to become healthy before removing the previous container
