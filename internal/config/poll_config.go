...
type PollConfig struct {
	...
	Deployments  []*DeployConfig  `yaml:"deployments"`                         // Deployments optionally define inline deployment configurations
}
...
func (c *PollConfig) Validate() error {
	...
	if len(c.Deployments) > 0 {
		for _, deploy := range c.Deployments {
			if deploy == nil {
				return fmt.Errorf("%w: deployment configuration is nil", ErrInvalidConfig)
			}

			if deploy.Reference == "" {
				deploy.Reference = c.Reference
			}

			if err := deploy.validateConfig(); err != nil {
				return fmt.Errorf("%w: %v", ErrInvalidConfig, err)
			}
		}

		if err := validator.Validate(c.Deployments); err != nil {
