...
func listIssuerRefs(ctx context.Context, client *api.Client, engineName string) ([]string, error) {
	pathToList := engineName + "/issuers"

	response, err := client.Logical().ListWithContext(ctx, pathToList)
	if err != nil {
		return nil, fmt.Errorf("unable to list issuers from OpenBao: %w", err)
	}
	...
	case []any:
		...
		return refs, nil
	default:
		return nil, errors.New("unexpected issuer list response")
	}
}
...
func parsePEMCertificate(value string) (*x509.Certificate, error) {
	...
	if block == nil {
		return nil, errors.New("value is not a PEM encoded certificate")
	}
