...
A push that changes the doco-cd compose file is a deployment like any other.
...
!!! warning "Pin the image"
    Use `#!yaml image: ghcr.io/kimdre/doco-cd:<version>@sha256:<digest>` and let a bot such as Renovate open the bump. 
    With a moving tag like `latest`, doco-cd upgrades itself whenever the tag moves and a breaking change lands unreviewed.

A demo setup can be found in [`examples/self-updating`](https://github.com/kimdre/doco-cd/tree/main/examples/self-updating).

## Requirements

A doco-cd stack must be defined in the deploy repository with the following rules:
...
- It must run exactly one container: `scale` or `replicas` is not supported.
...
The first container must already carry the labels of a managed stack, otherwise the running instance cannot recognize 
the stack as its own. Run the one-shot bootstrap once per host:

```sh title="Bootstrap script: bootstrap.sh"
--8<-- "examples/self-updating/bootstrap.sh"
```
...
1. Recreate the running instance one last time, the way it was started:
    - Set the image to a release that supports self-update.
    - Set `#!ini SELF_UPDATE_ENABLED=true`, a restart policy and a healthcheck.
...
3. In that compose file mount the same `data` volume as [`#!yaml external: true`](https://docs.docker.com/reference/compose-file/volumes/#external).
...
4. Push the changes to your deploy repository. The next time doco-cd does a deployment, it deploys the stack, 
   finds its own container in it and hands over to a successor created from the repository. 
   From then on the repository is the only source of truth.
...
Two containers cannot share a name or a host port. Drop both from the doco-cd service and reach the API through 
a reverse proxy on the compose network. Recreating a project network through the `applier` also briefly
interrupts other services attached to it and a failed update restores their previous state.
...
Docker restarts a crashing applier at most three times while the running instance still serves before the update fails. 
Once the running instance has handed over, Docker keeps restarting the applier until it finishes or
restores the previous container.

Once admission closes for a handover, new webhook and poll work is refused until service resumes. 
The predecessor waits for all in-flight deployments without a fixed timeout, so this can take longer than a few seconds. 
Webhook senders should retry requests that receive 503.

!!! note "Swarm"
    Docker Swarm already replaces a service through a rolling update performed by the swarm manager, 
    so a Swarm stack containing doco-cd updates itself without any of this.

## Alternative: two instances

The older approach runs two instances that deploy each other: a **main** instance handling your deployments, 
and an **updater** instance whose only job is redeploying the main one. 
It still works and needs no `SELF_UPDATE_ENABLED`, at the cost of a second container, 
a second clone of every repository and a second scheduler to keep disabled.
...
The updater polls with `#!yaml target: updater` and deploys the main instance. 
Set [`SCHEDULER_ENABLED`](../App-Settings.md) to `false` on the updater so both instances do not pick up the same scheduled jobs. 
If Docker reports a container name conflict during the handover, set `#!yaml force_recreate: true` for that stack or
