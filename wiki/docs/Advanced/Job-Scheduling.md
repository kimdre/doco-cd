...
| `cd.doco.job.stop_services.timeout` | integer | Positive timeout in seconds when stopping `stop_services` targets; see [stop timeout behavior](#stop-timeout-behavior)                                                                        | (see below) |
...
- **Standalone compose**: the container's `stop_grace_period` (compose file) is respected, letting the Docker engine apply it natively. If the target has no `stop_grace_period` configured, a default of 30 seconds is used, matching previous behavior.
- **Swarm**: the service's configured `stop_grace_period`, plus a small observation buffer, is used as the wait deadline so a long grace period is not treated as a timeout error prematurely. If unset, the default of 30 seconds is used.
