...
To enable the webhook endpoint, set the `WEBHOOK_SECRET` [environment variable](App-Settings.md#api-and-webhook-settings) to a secure secret value and publish the webhook port in the doco-cd `docker-compose.yml` file.
The default port is `80`; see the `HTTP_PORT` [environment variable](App-Settings.md#http-and-network-settings).
