...
|-----------------------------------|---------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------|
| `GIT_CLONE_DEPTH`                 | number  | Limits the number of commits fetched during clone/fetch operations (shallow clone). `0` means full clone (no depth limit). Can be overridden per deployment via the [`git_depth`](Deploy-Settings.md) setting. Synchronization fetches normally transfer only the requested ref; doco-cd automatically broadens the transfer when provider compatibility or reference reachability requires it. When a requested ref is outside the shallow depth, doco-cd deepens incrementally before falling back to a full fetch. | `0`                                              |
