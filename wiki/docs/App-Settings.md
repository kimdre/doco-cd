...
| `ARTIFACT_GC_ENABLED`           | boolean        | Enables the built-in sweeper that removes old, unreferenced published source artifacts (Git revisions/OCI digests) to reclaim disk space and limit how long decrypted SOPS secrets remain on disk. See [Artifact Garbage Collection](#artifact-garbage-collection).                                                                                                                                     | `true`                                                        |
...
- It is one of the `ARTIFACT_GC_RETENTION_RECORDS` most-recently-created copies for its
...
- It is younger than `ARTIFACT_GC_RETENTION_TTL` - even an unreferenced copy is not removed the
...
is removed. The sweep runs once at startup and then every `ARTIFACT_GC_INTERVAL`; it can be disabled
entirely with `ARTIFACT_GC_ENABLED: false` if you prefer to manage disk usage yourself.
