...
| `OCI_INSECURE_REGISTRIES`    | list    | Comma-separated OCI registry `host[:port]` entries for Compose includes. **TLS verification is disabled** for these registries; use only for trusted registries.                                                                                     | Ignored when not specified                      |
