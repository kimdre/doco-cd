...
| `TRUSTED_PROXY_HEADER`       | string  | HTTP header name containing the client's original IP address. Only used when the remote peer's IP is in `TRUSTED_PROXY_NETWORKS`. Header names are matched case-insensitively. | `X-Forwarded-For` (falls back to `Forwarded` header [RFC 7239](https://tools.ietf.org/html/rfc7239))                               |
