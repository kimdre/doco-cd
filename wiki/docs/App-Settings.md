...
| `RECOVER_ON_STARTUP`         | boolean        | Rebuilds reconciliation state and event listeners for already-deployed repositories on process start/restart, without waiting for the next poll cycle or webhook delivery. Reloads deploy configs directly from the existing local checkout on the data volume (no cloning/fetching for that step); repositories without a local checkout on the data volume are skipped and recovered on their next poll/webhook trigger instead. It then heals drift the same way a normal deploy would: restarts containers already unhealthy and redeploys stacks with no running containers/services at all, which can reach the network (git fetch/OCI pull) like any other reconciliation redeploy. | `true`                                                        |
