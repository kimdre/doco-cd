...
When using [custom webhook targets](Endpoints/Webhook-Listener.md#with-custom-target), nested config files always use the standard naming convention (`.doco-cd.yaml` / `.doco-cd.yml`), not the custom target name.
